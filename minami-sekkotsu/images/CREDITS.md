...
10枚とも [Unsplash](https://unsplash.com/) の写真です。
**Unsplash License は帰属表示を求めていません**（商用利用可・改変可）。
このページのスクリーンショットを出品ページや資料に使っても問題ありません。

下の一覧は記録です。Unsplash の API ガイドラインが撮影者と Unsplash への
リンクを推奨しているため、フッターにも一行載せています。

架空の店のサンプルなので、実案件では現地で撮影した写真に差し替える前提です。
...
`<video>` の `src` は空です。`images/hero.mp4` と `images/hero-poster.jpg` を
置いて指定します。
