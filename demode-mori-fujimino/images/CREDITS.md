...
このサイトの素材は画像9枚と映像1本。**うち2枚は demode MORI '67 さんご自身のInstagram投稿の実写、
残り7枚と映像1本は Higgsfield で生成したイメージ**です。Unsplash等のフリー素材は使っていません。

イメージ素材には、サイト本体の各素材の近くに「※画像はイメージです」「※背景の映像はイメージです」と
明記し、フッターにも実写でない旨とAI生成である旨を書いています。ご契約いただけましたら、
実際のお料理・店内を撮影して差し替えます。

**実在するこの店の外観・内観をAIで作り直すことは一切していません。** 生成した店内画像は、
店を特定しない一般的なカウンターのイメージで、ヒーローに「※背景の画像はイメージです。実際の店内を
撮影したものではありません。」と明記しています。

## 店ご自身の写真（実写・2枚）

| ファイル | 使用箇所 | 内容 | 出典 |
|---|---|---|---|
| `01-gaikan.jpg` | お店の情報 | 赤い庇の店名サインと入口 | 店Instagram [@demodemori67](https://www.instagram.com/demodemori67/) 2026-05-20投稿 |
| `04-unagi-salad.jpg` | About | 鰻のサラダ | 店Instagram [@demodemori67](https://www.instagram.com/demodemori67/) 2025-06-23投稿（#鰻のサラダ） |

`01-gaikan.jpg` にだけ後処理をしています。**写っているものは足しても引いてもいません**が、
Instagram由来で解像度が足りなかったため Higgsfield の `upscale_image`（bytedance・4K）で
4096×2323に引き上げ、サイトの配色（夕方から夜へ暮れる）に合わせて色を夕方寄りに整えました
（明るさ −7%・コントラスト +10%・暖色寄りのカラーバランス・弱いビネット）。撮影は昼間です。
...
- 実費: 合計71クレジット（画像は作り直し込みで12回生成＝24／実写のアップスケール2／映像45）
...
| ファイル | 使用箇所 | 内容 |
|---|---|---|
| `09-counter-yuugure.jpg` | ヒーロー（背景・Ken Burns） | 灯りがついたばかりの、何も置かれていないカウンター |
...
3. **`06-lanterns` の遠景に、遠近が破綻した提灯の一群があった**（遠いほうが手前より大きい）
   → 手前側だけを切り出して除去。
4. **ヒーローの候補2枚で、瓶のラベルに崩れた偽の漢字が出た**（1枚は実在銘柄に読めてしまう字面だった）
   → いずれも不採用。採用した `09-counter-yuugure.jpg` は瓶がすべて被写界深度の外にあり、
   等倍でも文字が像を結ばないことを確認済み。**カウンターに物を置かない構図にすると、
   箸置きの複製のような破綻も同時に消える**（置いた構図の候補では再びこの破綻が出た）。
...
## ヒーローを店内イメージにした理由（2026-08-12）

当初はヒーローに `01-gaikan.jpg`（実写の外観）を全面背景で敷いていたが、次の3点で成立していなかった。

1. **店名サインが「ORI '67」までしか写っておらず、文字が途中で切れていた。** すぐ下の見出しが
   「demode MORI '67」なので、二重に出たうえ片方が壊れて見える。**元のInstagram写真の時点で
   斜めから撮られていて左側が写っていないため、どう切り直しても直らない。**
2. 解像度が足りず、全面背景に引き伸ばすと輪郭が溶けていた（→ 4Kアップスケールで解決）。
3. **昼間の閉まった扉の写真**で、このサイトの背骨「夕方五時半から深夜へ暮れていく／SINCE THE
   EVENING」と正面から噛み合わなかった。

店のInstagram（215投稿）は告知画像が中心で、**夜の外観写真は存在しない**ことを確認した。
そこで実写の外観は「お店の情報」に等身大の大きさで移し、ヒーローには**店を特定しない
カウンターのイメージ**を置いた。実写を捨てたわけではなく、置き場所を変えている。

## 差し替え手順

ご契約後は、提携のカメラマンと撮影に伺い、生成した7枚を実際のお料理・店内の写真に差し替えます。
**ヒーローは、お店の外観か店内を実際に撮影したものに差し替えるのが最優先です。**
